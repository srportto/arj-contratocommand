...
public class PixAutoAutorizacaoService {
  ...
  public Autorizacao criar(ReceberAutorizacaoRequest request) {
    ...
    validarDataFimVigencia(request.dataFimVigencia());
    validarValorLimite(request.valor(), request.valorLimite());
    
    // Converter DTO para Entity
    Autorizacao autorizacao = mapper.toDomain(request);
  ...
  private void validarDataFimVigencia(LocalDate dataFimVigencia) {
    ...
    if (dataFimVigencia.isBefore(hoje)) {
      ...
      throw new BusinessException("A data de fim de vigência não pode ser no passado. Data informada: " + dataFimVigencia);
    }
  }
